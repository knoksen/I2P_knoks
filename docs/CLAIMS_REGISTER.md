...
| CLAIM-023 | Repeated imports of the same supported public identity are handled idempotently through a canonical fingerprint and database uniqueness constraint, with deterministic DAO and repository outcomes. | README, validation docs, release review notes, future onboarding | `IMPLEMENTED_TEST_VERIFIED` | `ConnectIdentityPublicCanonicalizer`, `ConnectIdentityFingerprint`, `ConnectIdentityDao.insertOrFindConnectIdentity`, `I2PRepository.importConnectIdentityPublic`, Room v6 unique `connect_identities.fingerprint` index. | `ConnectIdentityModelTest`, `ConnectIdentityRepositoryTest`, `I2PViewModelCoreFlowTest`, `LogSanitizerTest`; GitHub Actions `Android / identity-instrumentation` run `28837867825` for commit `894b8dd`; instrumentation XML reported `tests=5`, `failures=0`, `errors=0`, and `skipped=0`. | Duplicate detection establishes equality under the current canonical public-identity format; it does not verify ownership, trustworthiness, personhood, authenticated contacts, messaging readiness, secure key exchange, or absence of fingerprint collisions. Existing metadata is preserved on duplicate import. | "repeated imports of the same supported public identity are idempotent"; "duplicate public identity imports return the existing local row"; "canonical public identity duplicate handling is covered by JVM and Room/SQLite tests." | "verified identity"; "proof of ownership"; "trusted contact"; "secure identity exchange"; "collision impossible"; "private identity backup"; "anonymous messaging". | Reviewed 2026-07-07 | Ownership proof, trust establishment, collision-risk analysis beyond current fingerprinting, authenticated invites, and UX review for any contact-trust wording. |
