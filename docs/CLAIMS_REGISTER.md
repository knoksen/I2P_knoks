...
| CLAIM-010 | App settings and Connect identity migrations are present and have a bounded migration-test contract. | README, release docs | `PARTIALLY_VALIDATED` | Room migrations `MIGRATION_4_5`, `MIGRATION_5_6`; committed schemas for versions 4, 5, and 6; `AppDatabaseMigrationInstrumentedTest`. | `AppDatabaseMigrationTest`; instrumentation APK compile; CI emulator job configured for `AppDatabaseMigrationInstrumentedTest`. | JVM guard verifies schema files, graph, SQL shape, and no current destructive fallback. Full migration execution evidence requires an emulator/device or GitHub Actions run. | "Room schema files and migration graph are checked"; "instrumented migration tests are available for supported 4->5->6 paths." | "all historical databases can be recovered"; "migration is lossless"; "downgrades are supported"; "migration testing proves storage security". | Reviewed 2026-07-07 | Successful instrumented migration execution in CI/local emulator, plus claim review after reports are available. |
