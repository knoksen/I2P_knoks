...
REPORT_DIR="app/build/reports/connect-identity-ci"
mkdir -p "$REPORT_DIR"
...
test_exit=0
./gradlew connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=no.knoksen.i2pbrowser.ConnectIdentityImportInstrumentedTest 2>&1 | tee "$REPORT_DIR/gradle-output.txt" || test_exit=$?
